...
| `bt-backend-rustc` | **rustc 原生 LLVM 后端**（100% 兼容模式） | 1 |
| OpenTelemetry 全链路追踪 | 每个编译阶段有 Span | 1 |
| LSP 适配器 | VS Code 基本代码补全/诊断 | 1 |
| Dashboard + WebSocket | Web 监控仪表盘 | 0.5 |

**里程碑**：AI 辅助优化 + 三后端（LLVM/Cranelift/rustc 原生） + IDE 集成 + 实时监控全部可用
...
| TypeScript 前端 | **bt-ts2rs** 转译器（官方 TS 编译器 API → Rust 源码） | 3 |
| blocktype-ts-runtime | TS 运行时兼容库（Rust 函数层：console/Math/JSON/Date/Array） | 0.5 |
| 端到端集成测试 | Rust + TS 双前端测试（TS 走 ts2rs 转译） | 1 |
...
| Rust 原生后端 | cranelift | Rust 原生，WASM 支持好 |
| **100% 兼容后端** | **rustc 原生 LLVM** (bt-backend-rustc) | 委托 rustc 自身 codegen，100% 兼容、内联汇编/SIMD/sanitizers 全支持 |
| 插件系统 | WASM Component Model (WIT) | 沙箱隔离 + 多语言 + 正式接口 |
| 增量编译 | Salsa 风格查询引擎 | 依赖追踪 + 自动失效 |
| 事件系统 | Event Sourcing | 可回放 + AI 训练 + Dashboard |
| TS 前端 | **ts2rs 转译** (bt-ts2rs — TS→Rust 转译，复用完整 Rust 链路) | 与 Rust 前端同一策略（复用官方编译器），Day 1 生产级 |
| TS 运行时 | **blocktype-ts-runtime**（Rust 函数层兼容库，非 VM） | 轻量 Rust crate，而非自建 JS 运行时 |
| CLI | clap | Rust 标准选择 |
| 异步运行时 | tokio | 生态标准 |
...
## Phase 6+: 前瞻扩展（可选，按需追加）

> **定位**：Phase 0-5 完成（~45 周）后，BlockType Rust 全功能可用。
> 以下 12 项前瞻扩展可在主线交付后按需追加，每项 2-8 周。
> 详细方案见 `future-extensions/` 目录的各 Fxx 文档。

### 整合策略速查

| Phase | 推荐扩展 | 工期 | Task ID | 详细任务文件 |
|-------|---------|------|---------|------------|
| **Phase 2 扩展** | F01 分布式编译缓存（本地缓存层） | 3 周 | T-02-E01 ~ E02 | [DEV-03](./dev-plan/DEV-03-Phase2-Tasks.md) |
| | F02 供应链安全（策略引擎） | 2 周 | T-02-E03 ~ E04 | [DEV-03](./dev-plan/DEV-03-Phase2-Tasks.md) |
| | F09 交叉编译管理 | 2 周 | T-02-E05 ~ E06 | [DEV-03](./dev-plan/DEV-03-Phase2-Tasks.md) |
| **Phase 3 扩展** | F02 供应链安全（SBOM + SLSA） | 3 周 | T-03-E01 ~ E03 | [DEV-04](./dev-plan/DEV-04-Phase3-Tasks.md) |
| | F09 交叉编译（链接器配置） | 1 周 | T-03-E04 | [DEV-04](./dev-plan/DEV-04-Phase3-Tasks.md) |
| **Phase 4 扩展** | F05 持续编译（WatchService） | 3 周 | T-04-E01 ~ E03 | [DEV-05](./dev-plan/DEV-05-Phase4-Tasks.md) |
| | F08 绿色编译 | 1 周 | T-04-E06 ~ E07 | [DEV-05](./dev-plan/DEV-05-Phase4-Tasks.md) |
| | F06 AI 自适应策略（规则引擎版） | 2 周 | T-04-E04 ~ E05 | [DEV-05](./dev-plan/DEV-05-Phase4-Tasks.md) |
| **Phase 5 扩展** | F01 分布式编译缓存（远程缓存） | 3 周 | T-05-E01 ~ E03 | [DEV-06](./dev-plan/DEV-06-Phase5-Tasks.md) |
| | F06 AI 自适应策略（ML 模型版） | 3 周 | T-05-E04 ~ E05 | [DEV-06](./dev-plan/DEV-06-Phase5-Tasks.md) |
| | F11 编译器 LLM 协作 | 3 周 | T-05-E06 ~ E08 | [DEV-06](./dev-plan/DEV-06-Phase5-Tasks.md) |
| **Phase 6+** | F03 形式化验证 | 8 周 | T-06-F01 ~ F05 | [DEV-07](./dev-plan/DEV-07-Phase6-Tasks.md) |
| | F04 编译时 Fuzzing | 5 周 | T-06-F06 ~ F09 | [DEV-07](./dev-plan/DEV-07-Phase6-Tasks.md) |
| | F07 Compiler Marketplace | 5 周 | T-06-F10 ~ F12 | [DEV-07](./dev-plan/DEV-07-Phase6-Tasks.md) |
| | F10 浏览器内编译器 | 8 周 | T-06-F13 ~ F17 | [DEV-07](./dev-plan/DEV-07-Phase6-Tasks.md) |
| | F12 跨语言编译时反射 | 6 周 | T-06-F18 ~ F22 | [DEV-07](./dev-plan/DEV-07-Phase6-Tasks.md) |

### Phase 0-5 的可选扩展插入点

各主 Phases 末尾已增加详细 Task 分解，标注当前 Phase 完成后可追加的扩展项。
详见各 dev-plan 文件：
- [DEV-01-Phase0-Tasks.md](./dev-plan/DEV-01-Phase0-Tasks.md)（F08 绿色编译基础）
- [DEV-02-Phase1-Tasks.md](./dev-plan/DEV-02-Phase1-Tasks.md)（F12 跨语言反射基础）
- [DEV-03-Phase2-Tasks.md](./dev-plan/DEV-03-Phase2-Tasks.md)（F01/F02/F09 详细 Task）
- [DEV-04-Phase3-Tasks.md](./dev-plan/DEV-04-Phase3-Tasks.md)（F02/F09 详细 Task）
- [DEV-05-Phase4-Tasks.md](./dev-plan/DEV-05-Phase4-Tasks.md)（F05/F08/F06 详细 Task）
- [DEV-06-Phase5-Tasks.md](./dev-plan/DEV-06-Phase5-Tasks.md)（F01/F06/F11 详细 Task）
- [DEV-07-Phase6-Tasks.md](./dev-plan/DEV-07-Phase6-Tasks.md)（F03/F04/F07/F10/F12 详细 Task）
